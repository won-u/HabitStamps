...
**남은 후속 과제**: 이번 세션 동안 여러 기기(태블릿/폰)로 "제라도 301EXX"의 체크인 토글을 반복 테스트하면서, 원래 있던 중복(이미 SQL로 정리됨)과는 별개로 2026-08-10/11에 새로운 중복 row가 다시 쌓인 것으로 보인다(태블릿 로컬 DB 직접 확인, 서버 쪽 재확인 필요). 이번엔 실제 사용 이력(체크 해제 후 재체크 등)과 뒤섞여 있어 "가장 먼저 생성된 row만 남기고 나머지 삭제" 같은 단순 규칙을 함부로 다시 적용하면 진짜 체크인 기록을 지울 위험이 있다 — 사용자와 함께 정확한 날짜/row를 확인한 뒤 정리할 것.
