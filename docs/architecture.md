...
- **11차: 아이폰 PWA에서 한 번에 한 칸씩만 이동되던 문제 — 근본 원인은 드래그 도중 실제 배열/DOM을 재배치하고 있었던 것**: 안드로이드에서 여러 칸 연속 이동이 이미 `adb shell input draganddrop`으로 검증된 상태였는데도, 사용자가 아이폰 PWA(손잡이 드래그)에서는 "여러 칸 거리를 끌어도 한 칸만 옮겨지고, 계속 누르고 있어도 더 이상 진행되지 않는다"고 보고했다. 기존 구현은 `onDragMove`(임계값을 넘을 때마다 호출)에서 곧바로 `order` state를 스플라이스해 **드래그 도중에도 실제 배열 순서(=DOM 자식 순서)를 매번 바꾸고 있었다** — 네이티브는 RN 자체의 제스처 리스폰더가 뷰 재배치에 강건해서 문제가 없었지만, iOS Safari는 터치가 진행 중인 요소의 DOM 서브트리가 변경되면 그 터치 시퀀스를 취소하는 것으로 알려져 있다(`touchcancel`) — 첫 번째 재배치 직후 제스처가 종료돼버려 "한 칸만 이동하고 멈추는" 증상과 정확히 들어맞는다. 이 이론은 이 샌드박스에서 실제 iOS Safari로 직접 재현·확인하지는 못했다(WebKit 터치 이벤트 시뮬레이션 자체가 이전 조사에서 막다른 길이었음, §3-5 3번 항목 참고) — 실제 웹 드래그 재정렬 라이브러리들이 공통으로 쓰는 패턴(드래그 중엔 실제 배열을 건드리지 않고 시각적 transform만 적용, 손을 뗄 때 한 번만 실제로 반영)에 근거해 고쳤다. `reorderable-list.tsx`를 리팩터링: `ReorderableList`는 이제 드래그 중 목표 위치를 `dragToIndex`라는 순수 렌더링용 state로만 갖고 있고, 실제 스플라이스는 `onDragEnd`(=`commit`) 한 번뿐이다. 대신 드래그 시작 위치와 현재 목표 위치 사이에 있는 **다른** 행들은 끌리는 행의 높이만큼 `shiftY`(`useSharedValue` + `withSpring`)로 시각적으로만 밀려난다 — 실제로 손을 놓는 순간의 배열 재배치와 정확히 같은 결과를 미리 transform으로 보여주는 것. 이 설계 덕에 끌리는 행 자신의 `currentOffsetY`(자기 슬롯의 기준 위치)도 드래그가 끝날 때까지 절대 바뀌지 않게 돼서, 기존에 있던 "드래그 중 재배치로 점프한 위치를 보정"하던 `correction` 로직이 사실상 항상 0이 되도록 단순화됐다(10차의 `currentOffsetYShared` 크로스-드래그 최신값 동기화는 그대로 유지 — 두 번째 드래그를 시작할 때는 여전히 필요). 부작용: DOM/배열 순서가 드래그 중엔 전혀 안 바뀌므로 `LinearTransition` 레이아웃 애니메이션이 필요 없어져 제거했고(웹 전용 분기였던 것도 함께 정리), 웹에서도 이제 형제 행들이 부드럽게 자리를 비켜주는 애니메이션이 생겼다(예전엔 웹만 애니메이션 없이 순간이동했음 — 보너스 개선). 안드로이드 에뮬레이터에서 리팩터링 전후로 3칸 연속 드래그(빠른 것/느린 것 모두)를 다시 검증해 회귀가 없음을 확인했다 — 다만 이 항목의 핵심 주장인 "iOS Safari의 터치 취소가 진짜 원인이었다"는 것 자체는 실기기/실제 iOS 브라우저 테스트로 사용자가 직접 확인해줘야 최종 확정된다.
